...
def create_professional_chart(data, title, color, ma_windows=None, chart_height=400, line_shape="linear", show_markers=False, quick_zoom=True):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            rangeselector=dict(
                ...
                font=dict(color=COLORS['dark_text_secondary'])
...
def export_chart_as_png(fig, chart_title, quality="High"):
    """Export Plotly figure as PNG with professional settings"""
    try:
        ...
        st.info("💡 **Tip:** Install kaleido with `pip install kaleido` to enable PNG export")
        return None, None

def create_export_button(fig, chart_title, quality_setting):
    """Create a download button for chart export"""
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    hist = stock.history(start=start_date, end=end_date, interval="1d", repair=True)
...
def display_data_quality_metrics(performance_stats: dict, status_messages: list):
    """Creates a professional dashboard to show the health and performance of the data connections"""
...
smooth_lines = st.sidebar.checkbox(
    "📈 Smooth Lines", 
    value=False,
    help="Use spline curves for smoother, more professional-looking charts"
)

show_markers = st.sidebar.checkbox(
    "🔸 Show Data Points", 
    value=False,
    help="Display small markers at each data point on the lines"
)

enable_quick_zoom = st.sidebar.checkbox(
    "Quick Zoom Buttons", 
    value=True,
    help="Add 1M/3M/6M/1Y/All quick-select buttons on charts"
)
...
chart_quality = st.sidebar.selectbox(
    ...
    ["Standard", "High", "Ultra"],
    index=1,
    help="Chart resolution for PNG exports"
)
...
with st.spinner(f'🔄 Loading {len(selected_indicators)} indicators for {selected_range}...'):
    ...
    if use_live_data:
        df, status_messages, performance_stats = get_optimized_market_data(selected_indicators, days_back)
                ...
                if indicator_key in df.columns:
                    ...
                    with st.expander(f"📊 {indicator['name']} - Technical Analysis", expanded=True):
                        ...
                        if isinstance(chart, go.Figure):
                            ...
                            st.warning("⚠️ Unable to render chart — no data available.")
